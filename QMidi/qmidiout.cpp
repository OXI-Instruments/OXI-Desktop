...
unsigned int QMidiOut::getPortCount(void)
{
    return _midiOut->getPortCount();
}
...
void QMidiOut::openPort(unsigned int index, QString name)
{
    _midiOut->openPort(index, name.toStdString());
}
